...
async function fetchData() {
  try {
    const [t1, t2, h1, h2, w1, w2, aqi1, aqi2, m1] = await Promise.all([
      ...
      fetch(`https://blynk.cloud/external/api/get?token=${token}&V9`).then(r => r.text())  // Motion
    ]);
    ...
    const motionStatus = m1 === "1" ? "Detected" : "Not Detected";
    ...
    document.getElementById("motion-status").innerText = motionStatus;
